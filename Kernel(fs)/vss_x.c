...
//lw supported - kernel 2.6.18


MODULE_LICENSE("GPL");
MODULE_AUTHOR("IV-DEV");
MODULE_DESCRIPTION("Kernel module for supporting block device snapshots and incremental backups.");
MODULE_VERSION(VssX_VERSION);

//printing macros
#define LOG_DEBUG(fmt, args...) \
	do{ \
        if(vss_x_debug) printk(KERN_DEBUG "vss_x: " fmt "\n", ## args); \
	}while(0)

#define LOG_WARN(fmt, args...) printk(KERN_WARNING "vss_x: " fmt "\n", ## args)
#define LOG_ERROR(error, fmt, args...) printk(KERN_ERR "vss_x: " fmt ": %d\n", ## args, error)
...
#define vss_x_get_dentry(f) (f)->f_dentry
#define vss_x_get_mnt(f) (f)->f_vfsmnt
#else
#define vss_x_get_dentry(f) (f)->f_path.dentry
#define vss_x_get_mnt(f) (f)->f_path.mnt
#endif
...
#define vss_x_d_path(path, page_buf, page_size) d_path((path)->dentry, (path)->mnt, page_buf, page_size)
#define vss_x_get_nd_dentry(nd) (nd).dentry
#define vss_x_get_nd_mnt(nd) (nd).mnt
#else
#define vss_x_d_path(path, page_buf, page_size) d_path(path, page_buf, page_size)
#define vss_x_get_nd_dentry(nd) (nd).path.dentry
#define vss_x_get_nd_mnt(nd) (nd).path.mnt
#endif
...
#ifdef HAVE_BD_SUPER
#define vss_x_get_super(bdev) (bdev)->bd_super
#define vss_x_drop_super(sb)
#else
#define vss_x_get_super(bdev) get_super(bdev)
#define vss_x_drop_super(sb) drop_super(sb)
#endif

#ifndef HAVE_BLKDEV_GET_BY_PATH
struct block_device *vss_x_lookup_bdev(const char *pathname, fmode_t mode) {
	int r;
	...
	inode = vss_x_get_nd_dentry(nd)->d_inode;
	if (!inode) {
		r = -ENOENT;
		goto fail;
	}
...
static struct block_device *blkdev_get_by_path(const char *path, fmode_t mode, void *holder){
	...
	bdev = vss_x_lookup_bdev(path, mode);
	if(IS_ERR(bdev))
		return bdev;
...
static inline void vss_x_set_bio_ops(struct bio *bio, req_op_t op, unsigned op_flags){
	bio->bi_rw = 0;
	...
	#define vss_x_set_bio_ops(bio, op, flags) bio_set_op_attrs(bio, op, flags)
#endif

	#define bio_is_discard(bio) ((bio)->bi_rw & REQ_DISCARD)
	#define vss_x_submit_bio(bio) submit_bio(0, bio)
	#define vss_x_submit_bio_wait(bio) submit_bio_wait(0, bio)

static inline int vss_x_bio_op_flagged(struct bio *bio, unsigned int flag){
	return bio->bi_rw & flag;
}

static inline void vss_x_bio_op_set_flag(struct bio *bio, unsigned int flag){
	bio->bi_rw |= flag;
}

static inline void vss_x_bio_op_clear_flag(struct bio *bio, unsigned int flag){
	bio->bi_rw &= ~flag;
}
#else
...
static inline void vss_x_bio_op_clear_flag(struct bio *bio, unsigned int flag){
	...
	#endif
	#define vss_x_submit_bio(bio) submit_bio(bio)
	#define vss_x_submit_bio_wait(bio) submit_bio_wait(bio)
#endif
...
#ifdef HAVE_BIO_ENDIO_INT
static void vss_x_bio_endio(struct bio *bio, int err){
...
int vss_x_bdev_stack_limits(struct request_queue *t, struct block_device *bdev, sector_t start){
...
#define vss_x_bdev_stack_limits(queue, bdev, start) bdev_stack_limits(&(queue)->limits, bdev, start)
...
static int kern_path(const char *name, unsigned int flags, struct path *path){
	...
	if(!ret){
		path->dentry = vss_x_get_nd_dentry(nd);
		path->mnt = vss_x_get_nd_mnt(nd);
	}
	return ret;
}
#endif
...
#define vss_x_bioset_create(bio_size, bvec_size, scale) bioset_create(bio_size, bvec_size, BIOSET_NEED_BVECS)
#elif defined HAVE_BIOSET_CREATE_3
#define vss_x_bioset_create(bio_size, bvec_size, scale) bioset_create(bio_size, bvec_size, scale)
#else
#define vss_x_bioset_create(bio_size, bvec_size, scale) bioset_create(bio_size, scale)
#endif
...
static int vss_x_should_remove_suid(struct dentry *dentry)
{
...
#ifdef HAVE_BLKDEV_PUT_1
//#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
	#define vss_x_blkdev_put(bdev) blkdev_put(bdev);
#else
	#define vss_x_blkdev_put(bdev) blkdev_put(bdev, FMODE_READ);
#endif

#ifndef HAVE_PART_NR_SECTS_READ
//#if LINUX_VERSION_CODE < KERNEL_VERSION(3,6,0)
	#define vss_x_bdev_size(bdev) ((bdev)->bd_part->nr_sects)
#else
	#define vss_x_bdev_size(bdev) part_nr_sects_read((bdev)->bd_part)
#endif
...
static inline int vss_x_call_mrf(make_request_fn *fn, struct request_queue *q, struct bio *bio){
	return fn(q, bio);
}
...
static inline void vss_x_inode_lock(struct inode *inode){
	mutex_lock(&inode->i_mutex);
}

static inline void vss_x_inode_unlock(struct inode *inode){
	mutex_unlock(&inode->i_mutex);
}
#else
	#define vss_x_inode_lock inode_lock
	#define vss_x_inode_unlock inode_unlock
#endif
...
static inline ssize_t vss_x_kernel_read(struct file *filp, void *buf, size_t count, loff_t *pos){
...
static inline ssize_t vss_x_kernel_write(struct file *filp, const void *buf, size_t count, loff_t *pos){
...
static inline struct request_queue *vss_x_bio_get_queue(struct bio *bio){
...
static inline void vss_x_bio_set_dev(struct bio *bio, struct block_device *bdev){
#ifdef HAVE_BIO_BI_BDEV
//#if LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0)
	bio->bi_bdev = bdev;
#else
...
static inline void vss_x_bio_copy_dev(struct bio *dst, struct bio *src){
#ifdef HAVE_BIO_BI_BDEV
//#if LINUX_VERSION_CODE < KERNEL_VERSION(4,14,0)
...
//name macros
#define INFO_PROC_FILE "vss_x-info"
#define DRIVER_NAME "vss_x"
#define CONTROL_DEVICE_NAME "vss_x-srv"
#define SNAP_DEVICE_NAME "vss_x%d"
#define SNAP_COW_THREAD_NAME_FMT "vss_x_snap_cow%d"
#define SNAP_MRF_THREAD_NAME_FMT "vss_x_snap_mrf%d"
#define INC_THREAD_NAME_FMT "vss_x_inc%d"
...
#define tracer_for_each_full(dev, i) for(i = 0, dev = ACCESS_ONCE(snap_devices[i]); i < vss_x_max_snap_devices; i++, dev = ACCESS_ONCE(snap_devices[i]))

//returns true if tracing struct's base device queue matches that of bio
#define tracer_queue_matches_bio(dev, bio) (bdev_get_queue((dev)->sd_base_dev) == vss_x_bio_get_queue(bio))
...
#define __VssX_PASSTHROUGH 30    /* don't perform COW operation */
#define VssX_PASSTHROUGH (1ULL << __VssX_PASSTHROUGH)

#define VssX_DEFAULT_SNAP_DEVICES 24
#define VssX_MAX_SNAP_DEVICES 255

//global module parameters
static int vss_x_may_hook_syscalls = 1;
static unsigned long vss_x_cow_max_memory_default = (300 * 1024 * 1024);
static unsigned int vss_x_cow_fallocate_percentage_default = 10;
static unsigned int vss_x_max_snap_devices = VssX_DEFAULT_SNAP_DEVICES;
static int vss_x_debug = 0;

module_param_named(may_hook_syscalls, vss_x_may_hook_syscalls, int, S_IRUGO);
...
module_param_named(cow_max_memory_default, vss_x_cow_max_memory_default, ulong, 0);
MODULE_PARM_DESC(cow_max_memory_default, "default maximum cache size (in bytes)");

module_param_named(cow_fallocate_percentage_default, vss_x_cow_fallocate_percentage_default, uint, 0);
MODULE_PARM_DESC(cow_fallocate_percentage_default, "default space allocated to the cow file (as integer percentage)");

module_param_named(max_snap_devices, vss_x_max_snap_devices, uint, S_IRUGO);
MODULE_PARM_DESC(max_snap_devices, "maximum number of tracers available");

module_param_named(debug, vss_x_debug, int, S_IRUGO | S_IWUSR);
...
static int vss_x_proc_show(struct seq_file *m, void *v);
static void *vss_x_proc_start(struct seq_file *m, loff_t *pos);
static void *vss_x_proc_next(struct seq_file *m, void *v, loff_t *pos);
static void vss_x_proc_stop(struct seq_file *m, void *v);
static int vss_x_proc_open(struct inode *inode, struct file *filp);
static int vss_x_proc_release(struct inode *inode, struct file *file);
...
static const struct seq_operations vss_x_seq_proc_ops = {
	.start = vss_x_proc_start,
	.next = vss_x_proc_next,
	.stop = vss_x_proc_stop,
	.show = vss_x_proc_show,
};

static const struct file_operations vss_x_proc_fops = {
	.owner = THIS_MODULE,
	.open = vss_x_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = vss_x_proc_release,
};
...
static int file_open(const char *filename, int flags, struct file **filp){
	...
	if(!f){
		...
		goto error;
	}else if(!S_ISREG(vss_x_get_dentry(f)->d_inode->i_mode)){
		ret = -EINVAL;
...
static int path_get_absolute_pathname(const struct path *path, char **buf, int *len_res){
	...
	pathname = vss_x_d_path(path, page_buf, PAGE_SIZE);
	if(IS_ERR(pathname)){
		ret = PTR_ERR(pathname);
		pathname = NULL;
...
static int file_get_absolute_pathname(const struct file *filp, char **buf, int *len_res){
	...
	path.mnt = vss_x_get_mnt(filp);
	path.dentry = vss_x_get_dentry(filp);
...
static int file_io(struct file *filp, int is_write, void *buf, sector_t offset, unsigned long len){
	...
	if(is_write) ret = vss_x_kernel_write(filp, buf, len, &off);
	else ret = vss_x_kernel_read(filp, buf, len, &off);
...
static int vss_x_do_truncate(struct dentry *dentry, loff_t length, unsigned int time_attrs, struct file *filp){
	int ret;
	...
	ret = vss_x_should_remove_suid(dentry);
	if(ret) newattrs.ia_valid |= ret | ATTR_FORCE;

	vss_x_inode_lock(dentry->d_inode);
...
#endif
	vss_x_inode_unlock(dentry->d_inode);
...
//#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0)
	...
	ret = vss_x_do_truncate(dentry, len, ATTR_MTIME|ATTR_CTIME, filp);
...
static int real_fallocate(struct file *f, uint64_t offset, uint64_t length){
	...
	struct inode *inode = vss_x_get_dentry(f)->d_inode;
	#else
...
static int __file_unlink(struct file *filp, int close, int force){
	int ret = 0;
	struct inode *dir_inode = vss_x_get_dentry(filp)->d_parent->d_inode;
	struct dentry *file_dentry = vss_x_get_dentry(filp);
	struct vfsmount *mnt = vss_x_get_mnt(filp);
...
static int bio_make_read_clone(struct bio_set *bs, struct tracing_params *tp, struct bio *orig_bio, sector_t sect, unsigned int pages, struct bio **bio_out, unsigned int *bytes_added){
	...
	vss_x_bio_copy_dev(new_bio, orig_bio);
	vss_x_set_bio_ops(new_bio, REQ_OP_READ, 0);
...
static int snap_handle_read_bio(const struct snap_device *dev, struct bio *bio){
	...
	vss_x_bio_set_dev(bio, dev->sd_base_dev);
	vss_x_set_bio_ops(bio, REQ_OP_READ, READ_SYNC);
...
static int snap_mrf_thread(void *data){
	...
	while(!kthread_should_stop() || !bio_queue_empty(bq)) {
		...
		vss_x_bio_op_set_flag(bio, VssX_PASSTHROUGH);

		ret = vss_x_call_mrf(dev->sd_orig_mrf, vss_x_bio_get_queue(bio), bio);
#ifdef HAVE_MAKE_REQUEST_FN_INT
...
static int snap_cow_thread(void *data){
	...
	while(!kthread_should_stop() || !bio_queue_empty(bq) || atomic64_read(&dev->sd_submitted_cnt) != atomic64_read(&dev->sd_received_cnt)) {
		...
		if(!bio_data_dir(bio)){
			...
			if(is_failed){
				vss_x_bio_endio(bio, -EIO); //end the bio with an IO error
				continue;
			}
...
static void __on_bio_read_complete(struct bio *bio, int err){
	...
	vss_x_set_bio_ops(bio, REQ_OP_WRITE, 0);
...
static int snap_trace_bio(struct snap_device *dev, struct bio *bio){
	...
	if(!bio_needs_cow(bio, dev->sd_cow_inode)) return vss_x_call_mrf(dev->sd_orig_mrf, vss_x_bio_get_queue(bio), bio);
...
static MRF_RETURN_TYPE tracing_mrf(struct request_queue *q, struct bio *bio){
	...
	tracer_for_each(dev, i){
		...
		if(vss_x_bio_op_flagged(bio, VssX_PASSTHROUGH)){
		    vss_x_bio_op_clear_flag(bio, VssX_PASSTHROUGH);
			goto call_orig;
		}
...
call_orig:
	if(orig_mrf) ret = vss_x_call_mrf(orig_mrf, q, bio);
...
static MRF_RETURN_TYPE snap_mrf(struct request_queue *q, struct bio *bio){
	...
	if(bio_data_dir(bio)){
		vss_x_bio_endio(bio, -EOPNOTSUPP);
		MRF_RETURN(0);
	}else if(tracer_read_fail_state(dev)){
		vss_x_bio_endio(bio, -EIO);
		MRF_RETURN(0);
	}else if(!test_bit(ACTIVE, &dev->sd_state)){
		vss_x_bio_endio(bio, -EBUSY);
		MRF_RETURN(0);
	}
...
static int __tracer_transition_tracing(struct snap_device *dev, struct block_device *bdev, make_request_fn *new_mrf, struct snap_device **dev_ptr){
	int ret;
	struct super_block *origsb = vss_x_get_super(bdev);
...
static void __tracer_destroy_base_dev(struct snap_device *dev){
	...
	if(dev->sd_base_dev){
		...
		vss_x_blkdev_put(dev->sd_base_dev);
...
static int __tracer_setup_base_dev(struct snap_device *dev, const char *bdev_path){
	...
	if(dev->sd_base_dev->bd_contains != dev->sd_base_dev){
		...
		dev->sd_size = vss_x_bdev_size(dev->sd_base_dev);
	}else{
		dev->sd_sect_off = 0;
...
static int file_is_on_bdev(const struct file *file, struct block_device *bdev) {
	struct super_block *sb = vss_x_get_super(bdev);
	int ret = 0;
	if (sb) {
		ret = ((vss_x_get_mnt(file))->mnt_sb == sb);
		vss_x_drop_super(sb);
	}
	return ret;
}
...
static int __tracer_setup_cow(struct snap_device *dev, struct block_device *bdev, const char *cow_path, sector_t size, unsigned long fallocated_space, unsigned long cache_size, const uint8_t *uuid, uint64_t seqid, int open_method){
	...
	if(open_method == 3){
		...
		if(!cache_size) dev->sd_cache_size = vss_x_cow_max_memory_default;
		else dev->sd_cache_size = cache_size;

		if(open_method == 0){
			//calculate how much space should be allocated to the cow file
			if(!fallocated_space){
				max_file_size = size * SECTOR_SIZE * vss_x_cow_fallocate_percentage_default;
	...
	dev->sd_cow_inode = vss_x_get_dentry(dev->sd_cow->filp)->d_inode;
...
static int __tracer_setup_cow_path(struct snap_device *dev, const struct file *cow_file){
	...
	ret = dentry_get_relative_pathname(vss_x_get_dentry(cow_file), &dev->sd_cow_path, NULL);
	if(ret) goto error;
...
#ifndef HAVE_BIOSET_INIT
//#if LINUX_VERSION_CODE < KERNEL_VERSION(4,18,0)
	dev->sd_bioset = vss_x_bioset_create(BIO_SET_SIZE, BIO_SET_SIZE, 0);
...
static int __tracer_setup_snap(struct snap_device *dev, unsigned int minor, struct block_device *bdev, sector_t size){
	...
	vss_x_bdev_stack_limits(dev->sd_queue, bdev, 0);
...
static void minor_range_recalculate(void){
	unsigned int i, highest = 0, lowest = vss_x_max_snap_devices - 1;
	struct snap_device *dev;
...
static void tracer_reconfigure(struct snap_device *dev, unsigned long cache_size){
	dev->sd_cache_size = cache_size;
	if(!cache_size) cache_size = vss_x_cow_max_memory_default;
	if(test_bit(ACTIVE, &dev->sd_state)) cow_modify_cache_size(dev->sd_cow, cache_size);
}

static void tracer_vss_x_info(const struct snap_device *dev, struct vss_x_info *info){
	info->minor = dev->sd_minor;
	info->state = dev->sd_state;
	info->error = tracer_read_fail_state(dev);
	info->cache_size = (dev->sd_cache_size)? dev->sd_cache_size : vss_x_cow_max_memory_default;
...
static int __verify_minor(unsigned int minor, int mode){
	...
	if(minor >= vss_x_max_snap_devices){
...
static int __verify_bdev_writable(const char *bdev_path, int *out){
	...
	sb = vss_x_get_super(bdev);
	if(sb){
		writable = !(sb->s_flags & MS_RDONLY);
		vss_x_drop_super(sb);
	}

	vss_x_blkdev_put(bdev);
	*out = writable;
	return 0;
}
...
static int ioctl_vss_x_info(struct vss_x_info *info){
	int ret;
	struct snap_device *dev;

	LOG_DEBUG("received vss_x info ioctl - %u", info->minor);
...
static long ctrl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg){
	...
	case IOCTL_VSS_INFO:
		//get params from user space
	    info = kmalloc(sizeof(struct vss_x_info), GFP_KERNEL);
		if(!info){
			ret = -ENOMEM;
			LOG_ERROR(ret, "error allocating memory for vss_x info");
			break;
		}

		ret = copy_from_user(info, (struct vss_x_info __user *)arg, sizeof(struct vss_x_info));
		if(ret){
			ret = -EFAULT;
			LOG_ERROR(ret, "error copying vss_x info struct from user space");
			break;
		}

		ret = ioctl_vss_x_info(info);
		if(ret) break;

		ret = copy_to_user((struct vss_x_info __user *)arg, info, sizeof(struct vss_x_info));
		if(ret){
			ret = -EFAULT;
			LOG_ERROR(ret, "error copying vss_x info struct to user space");
			break;
		}
...
static int __handle_bdev_mount_nowrite(const struct vfsmount *mnt, unsigned int *idx_out){
	...
	tracer_for_each(dev, i){
		...
		if(mnt == vss_x_get_mnt(dev->sd_cow->filp)){
...
static void post_umount_check(int dormant_ret, long umount_ret, unsigned int idx, const char __user *dir_name){
	...
	sb = vss_x_get_super(dev->sd_base_dev);
	if(sb){
...
static int vss_x_proc_show(struct seq_file *m, void *v){
	...
	if(dev_ptr != SEQ_START_TOKEN && *dev_ptr != NULL){
		...
		seq_printf(m, "\t\t\t\"max_cache\": %lu,\n", (dev->sd_cache_size)? dev->sd_cache_size : vss_x_cow_max_memory_default);
...
static inline void *vss_x_proc_get_idx(loff_t pos){
...
static void *vss_x_proc_start(struct seq_file *m, loff_t *pos){
	...
	return vss_x_proc_get_idx(*pos - 1);
}

static void *vss_x_proc_next(struct seq_file *m, void *v, loff_t *pos){
	void *dev = vss_x_proc_get_idx(*pos);
	++*pos;
	return dev;
}
...
static int vss_x_proc_open(struct inode *inode, struct file *filp){
	mutex_lock(&ioctl_mutex);
	return seq_open(filp, &vss_x_seq_proc_ops);
}
...
static int __init agent_init(void)
{
	...
	//init minor range
	if(vss_x_max_snap_devices == 0 || vss_x_max_snap_devices > VssX_MAX_SNAP_DEVICES){
	    const unsigned int nr_devices = vss_x_max_snap_devices == 0 ? VssX_DEFAULT_SNAP_DEVICES : VssX_MAX_SNAP_DEVICES;
		LOG_WARN("invalid number of snapshot devices (%u), setting to %u", vss_x_max_snap_devices, nr_devices);
		vss_x_max_snap_devices = nr_devices;
	}

	highest_minor = 0;
	lowest_minor = vss_x_max_snap_devices - 1;
	...
	snap_devices = kzalloc(vss_x_max_snap_devices * sizeof(struct snap_device*), GFP_KERNEL);
	if(!snap_devices){
		ret = -ENOMEM;
	...
	info_proc = proc_create(INFO_PROC_FILE, 0, NULL, &vss_x_proc_fops);
	if(!info_proc)
	{
		ret = -ENOENT;
	...
	if(vss_x_may_hook_syscalls) (void)hook_system_call_table();
